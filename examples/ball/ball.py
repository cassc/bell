...
def make_face():
    ...
    iright = (x + 15, y-15, 10)
    mouth = (x, y+20, 10, 5)
    a = ui.make_circle(x, y, r, clear=False, color='white')
...
def draw_bouncing_ball(ctx):
    ...
    def append_shadows(sq, down):
        for i in range(1, 10, 2):
            ...
            y_offset *= i
            sq.append(ui.make_circle(x, y + y_offset, r, fill=True, color='rgba(50,100,120,{})'.format(1-0.1*i),  clear=False))
    ...
    for ball in balls:
        sq = list(face)
        sq.append(ui.make_circle(*ball, fill=True, color='yellow'))
